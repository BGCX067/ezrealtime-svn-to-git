...
int isEnabled(int M[], int t) {
	int i;
	for (i=0; i<pre[t].card; i++)
		if (M[pre[t].lugar[i]] < pre[t].peso[i])
			return FALSE;
	return TRUE;
}
...
void enabledTrans(struct state *S) {
	...
	for (i=0; i<NUM_TRANS; i++) {
		...
		for (j=0; j<pre[i].card; j++) {
			...
			if ((S->M[pre[i].lugar[j]] < pre[i].peso[j])) {
				enable = FALSE;
			}
...
void newMarking(int M[], int newM[], int t) {
	...
	if (isEnabled(M, t)) {
		for (i=0; i<pre[t].card; i++)
			newM[pre[t].lugar[i]] = newM[pre[t].lugar[i]] - pre[t].peso[i];
		for (i=0; i<post[t].card; i++)
			newM[post[t].lugar[i]] = newM[post[t].lugar[i]] + post[t].peso[i];
	} else {
...
void generateStateSlim(struct state *newS) {
	...
	int i;
	newS->state_s = malloc(sizeof(struct state_slim));
	...
	newS->state_s->M = malloc(sizeof(int) * NUM_PLACE);
...
int TRANS_DEADLINE(int t) {
	return TD.trans[BUSCA_POS_GRANT_PROCESSOR(t)];
}
